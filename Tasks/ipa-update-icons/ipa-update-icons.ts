...
async function run() {
    try {
        ...
        if (cwd == undefined) {
            throw Error("Unable to generate icons");
        }
        if (!tl.exist(cwd)) {
            tl.mkdirP(cwd);
        }
        ...
        let assetsFolder: string | undefined = tl.getPathInput('assetsPath');
        let appIconName: string | undefined = tl.getInput('appIconName');
        if (iconPath == undefined) {
            throw Error("Icon path undefined");
        }
        ...
        if (fromIPA) {
            ...
            let ipaOutputPath = await ipaUtilities.unpackIpa(ipaPath as string, path.join(cwd, outputPath));
            ...
            let repackedIpa = await ipaUtilities.packIpa(ipaOutputPath, path.basename(ipaPath as string), path.join(cwd, outputPath));
